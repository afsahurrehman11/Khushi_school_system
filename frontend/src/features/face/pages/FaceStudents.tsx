/**
 * Face App - Students Management Page
 * Formal design with simple non-technical buttons
 */
...
import type { StudentFace, ClassInfo, GenerateResult } from '../types';
import Button from '../../../components/Button';
import Modal from '../../../components/Modal';
import logger from '../../../utils/logger';

const FaceStudents: React.FC = () => {
  ...
  const fetchClasses = async () => {
    try {
      const data = await getClassesForFace();
      setClasses(data.classes || []);
    } catch (err) {
      logger.error('FACE UI', `Failed to fetch classes: ${err}`);
    }
  };
  ...
  const fetchStudents = async () => {
    setLoading(true);
    try {
      const data = await getStudentsForFace(selectedClass || undefined, statusFilter);
      setStudents(data.students || []);
    } catch (err) {
      logger.error('FACE UI', `Failed to fetch students: ${err}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePrepareMissing = async () => {
    ...
    logger.info('FACE UI', 'Preparing missing faces...');

    try {
      const result = await generateMissingEmbeddings('student', selectedClass || undefined);
  ...
  const handleRefreshAll = async () => {
    ...
    setOperationResult(null);
    logger.info('FACE UI', 'Refreshing all faces...');

    try {
      const result = await refreshAllEmbeddings('student', selectedClass || undefined);
      setOperationResult(result);
  ...
  const openUploadModal = (student: StudentFace) => {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    try {
      const result = await uploadFaceImage('student', selectedStudent.id, capturedImage);
      logger.info('FACE UI', `Image uploaded for ${selectedStudent.student_id}`);
  ...
  const handleRegenerate = async (student: StudentFace) => {
    try {
      await regenerateSingleEmbedding('student', student.id);
      logger.info('FACE UI', `Regenerated embedding for ${student.student_id}`);
      await fetchStudents();
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-secondary-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-secondary-900">Student Face Registration</h1>
              <p className="text-secondary-500 text-sm mt-1">
                Manage face registration for students
              </p>
            </div>
              ...
              <table className="w-full">
                <thead className="bg-secondary-50 border-b border-secondary-200">
                  <tr>
                    ...
                    <th className="text-left py-3 px-4 text-sm font-medium text-secondary-600">ID</th>
                    <th className="text-left py-3 px-4 text-sm font-medium text-secondary-600">Class</th>
                  ...
                  {filteredStudents.map((student) => (
                    <tr key={student.id} className="border-b border-secondary-100 hover:bg-secondary-50">
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-secondary-100 overflow-hidden flex items-center justify-center">
                          ...
                          <span className="font-medium text-secondary-900">{student.full_name}</span>
                        </div>
                      </td>
                      <td className="py-3 px-4 text-secondary-600">{student.student_id}</td>
                      <td className="py-3 px-4 text-secondary-600">
                        {student.class_id} - {student.section}
                      </td>
                      <td className="py-3 px-4">{getStatusBadge(student.embedding_status)}</td>
                      <td className="py-3 px-4">
      ...
      <Modal
        ...
        title={`Update Photo - ${selectedStudent?.full_name}`}
        size="md"
      >
